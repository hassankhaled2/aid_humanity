...
class _TestGoogleMapsState extends State<TestGoogleMaps> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: GoogleMap(
        markers:  Set.from([Marker(markerId: const MarkerId("1"), position: LatLng(widget.position.latitude, widget.position.latitude))]),
        initialCameraPosition: CameraPosition(target: LatLng(widget.position.latitude, widget.position.latitude)),
      ),
    );
  }
}
